"""
Databricks Procurement Accelerator — Spend Review
Human-in-the-loop invoice classification review tool.
"""

import streamlit as st

st.set_page_config(
    page_title="Spend Review · Databricks Procurement Accelerator",
    page_icon="🔎",
    layout="wide",
...
from src.app.queries import get_invoice_with_categorization
...
def _resolve_l1(config, category_l2: str) -> str:
    """Derive L1 category from an L2 name using the config hierarchy."""
    for l1, l2_dict in config.categories.items():
        if category_l2 in l2_dict:
            return l1
    return ""
...
def handle_submit_reviews(reviews: List[Dict]):
    ...
    try:
        ...
        formatted_reviews = []
        for r in reviews:
            reviewed_l2 = r["corrected_category"]
            reviewed_l1 = _resolve_l1(config, reviewed_l2)

            formatted_reviews.append(
                {
                    "order_id": r["order_id"],
                    "source": config.categorization_source,
                    "reviewer": config.default_user,
                    "original_level_1": r.get("original_level_1", ""),
                    "original_level_2": r.get("original_level_2", ""),
                    "original_level_3": r.get("original_level_3", ""),
                    "reviewed_level_1": reviewed_l1,
                    "reviewed_level_2": reviewed_l2,
                    "reviewed_level_3": "",
...
def render_sidebar():
    """Render sidebar with Databricks assets and review progress."""
    from src.app.sidebar import render_databricks_sidebar
    config = st.session_state.config
    render_databricks_sidebar(config)

    with st.sidebar:
        if st.session_state.total_invoices_in_review > 0:
            st.divider()
            with st.expander("Review Progress", expanded=False):
                ...
                st.write(f"**Done:** {done} | **Remaining:** {remaining}")
                if total > 0:
...
def main():
    ...
    with hdr1:
        st.markdown(
            """
            <div style="font-size:0.7rem;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:#FF3621;margin-bottom:4px;">Spend Review</div>
            <div style="font-size:1.4rem;font-weight:700;color:#0B2026;margin-bottom:0;">Invoice Classification Review Tool</div>
            """,
            unsafe_allow_html=True,
        )
    with hdr2:
