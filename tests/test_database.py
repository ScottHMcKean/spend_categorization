...
from invoice_app.config import AppConfig, LakebaseConfig
...
@pytest.fixture
def mock_backend(test_app_config):
    """Initialize and return a MockBackend."""
    reset_backend()
    backend = init_backend(test_app_config, None)
    yield backend
    reset_backend()
...
class TestCreateBackend:
    ...
    def test_create_mock_backend_for_test_mode(self):
        ...
        backend = create_backend(app_config)
        assert isinstance(backend, MockBackend)

    def test_prod_mode_requires_lakebase_config(self):
        """Test that prod mode requires LakebaseConfig."""
        app_config = AppConfig(
            invoices_table="invoices",
            corrections_table="invoice_corrections",
            mode="prod",
        )
        with pytest.raises(ValueError, match="LakebaseConfig is required"):
            create_backend(app_config, None)


class TestQueries:
    """Tests for query functions with MockBackend."""

    def test_get_available_categories(self, mock_backend, test_app_config):
        """Test getting available categories."""
        categories = get_available_categories(test_app_config, mock_backend)
        assert isinstance(categories, list)
        assert len(categories) > 0
        assert all(isinstance(c, str) for c in categories)

    def test_get_flagged_invoices(self, mock_backend, test_app_config):
        """Test getting flagged invoices."""
        flagged = get_flagged_invoices(test_app_config, limit=10, backend=mock_backend)
    ...
    def test_search_invoices(self, mock_backend, test_app_config):
    ...
    def test_get_invoices_by_ids(self, mock_backend, test_app_config):
        ...
        results = get_invoices_by_ids(test_app_config, test_ids, backend=mock_backend)
...
class TestCorrections:
    ...
    def test_write_correction(self, mock_backend, test_app_config):
    ...
    def test_write_corrections_batch(self, mock_backend, test_app_config):
        ...
        write_corrections_batch(test_app_config, corrections, backend=mock_backend)
...
class TestBackendGlobalState:
    ...
    def test_init_backend_sets_global(self, test_app_config):
